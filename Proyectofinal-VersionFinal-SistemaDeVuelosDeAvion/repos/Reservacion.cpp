...
Reservacion::Reservacion(int idUsuario,
    const Vuelo& vuelo,
...
const std::string& Reservacion::getAsiento() const {
    return asiento;
}

double Reservacion::getCosto() const {
    return costo;
}
...
Reservacion Reservacion::deserializar(const std::string& linea) {
    ...
    std::string asiento = campos[7];
    double costo = std::stod(campos[8]);

    Vuelo vuelo(idVuelo, fecha, hora, origen, destino, numeroAvion);
    return Reservacion(idUsuario, vuelo, asiento, costo);
}
