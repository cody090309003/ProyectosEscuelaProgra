...
bool procesarPago() {
    std::cout << "Simulando pago en línea...\n";
    std::cout << "Pago realizado correctamente.\n";
    return true;
}

bool esAdmin(const std::shared_ptr<Usuario>& u) {
    // Intentamos convertir a Admin
...
int main() {
    ...
    do {
        ...
        if (opcion == 1) {
            ...
            std::cout << "\nResumen de tu vuelo:\n";
            std::cout << vuelo.getDescripcionCorta() << " | Avión: " << vuelo.getNumeroAvion() << "\n";

            if (procesarPago()) {
                Reservacion r(idUsuario, vuelo);
